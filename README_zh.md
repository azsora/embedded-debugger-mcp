...
- 🧪 **全面测试**: 所有24个工具在真实STM32G431CBTx硬件上验证通过
...
- ✅ **全部 24 个 MCP 工具**: 在真实 STM32 硬件上完整验证
- ✅ **斐波那契计算器**: 实时数据流与控制命令
...
#### 测试全部 24 个 MCP 工具
```
请帮我测试所有 24 个 MCP 嵌入式调试工具与我的 STM32 开发板。先连接到探针，然后系统性地测试每个工具类别：探针管理、内存操作、调试控制、断点、Flash 操作、RTT 通信和会话管理。
```
...
| 工具 | 描述 | 状态 |
|------|------|------|
| `read_symbol` | 从ELF文件读取符号地址 | ✅ 生产就绪 |
| `read_variable` | 通过DWARF调试信息读取变量值 | ✅ 生产就绪 |
...
- ✅ **完整的 probe-rs 集成**: 所有24个工具的真实硬件调试
